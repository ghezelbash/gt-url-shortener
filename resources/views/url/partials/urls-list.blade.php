            ...
            @foreach ($urls as $url)
                <div class="font-mono flex justify-between items-center bg-gray-100 dark:bg-gray-900 p-2 rounded">
                    ...
                    <div class="w-2/6 text-gray-900 dark:text-gray-100 truncate">
                        <a href="{{ url($url->shortened_url) }}" class="text-blue-600 dark:text-blue-400">{{ url($url->shortened_url) }}</a>
                    </div>
                    <div class="w-1/6 flex items-center space-x-4">
                        <i class="fas fa-copy cursor-pointer text-gray-600 dark:text-gray-400" onclick="copyToClipboard('{{ url($url->shortened_url) }}', this)"></i>
